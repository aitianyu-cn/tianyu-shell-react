 ...
 */
export function setI18nModuleCache(lang: string, module: __WebpackModuleApi.RequireContext): void {
    _i18nModuleCache[lang] = module;
}
 ...
 */
export function getI18nModuleCache(lang: string): __WebpackModuleApi.RequireContext | undefined {
    return _i18nModuleCache[lang];
}
...
function getText(key: string, lang: string, params?: (string | number)[] | string): string | null {
    const requireContext = getI18nModuleCache(lang);
    const fileName = `./message${lang !== DEFAULT_LANGUAGE ? `_${lang}` : ""}.properties`;
    let resource;
    try {
 ...
 */
export function getTextFromFile(key: string, params?: (string | number)[] | string, fallString?: string): string {
    return getText(key, Language.toString(), params) || getText(key, DEFAULT_LANGUAGE, params) || fallString || key;
}
